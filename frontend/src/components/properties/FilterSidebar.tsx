...
import type { PropertyFilters } from '../../pages/PropertiesPage';

/** Slider 0–200 maps to amount in ETB (same multiplier as PropertiesPage filter). */
const SLIDER_TO_ETB = (units: number) => units * 1_000_000;
...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ onFilterChange }) => {
  ...
  // Auto-apply filters whenever any filter state changes (syncs to PropertiesPage)
  useEffect(() => {
  ...
  return (
    <div className="w-full lg:w-[359px] bg-white border-b lg:border-b-0 lg:border-r border-[#E6E0DA] lg:h-[calc(100vh-5rem)] lg:sticky lg:top-20 overflow-y-visible lg:overflow-y-auto pb-10 lg:pb-24">
      <div className="p-4 sm:p-6">
        ...
        <div className="mb-8 border-b border-[#F5F1E8] pb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="font-space-mono text-xs sm:text-sm text-[#D4755B] text-right max-w-[min(100%,14rem)] leading-tight">
              {formatPrice(SLIDER_TO_ETB(priceRange[0]))} – {formatPrice(SLIDER_TO_ETB(priceRange[1]))}
            </span>
          </div>
          ...
          <div className="flex justify-between px-1 gap-2">
            <span className="font-manrope text-xs text-[#9CA3AF]">{formatPrice(0)}</span>
            <span className="font-manrope text-xs text-[#9CA3AF]">{formatPrice(SLIDER_TO_ETB(200))}+</span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            {propertyTypes.map((type) => (
              <button
                ...
                className={`h-[72px] sm:h-[80px] rounded-xl border flex flex-col items-center justify-center gap-1 sm:gap-2 transition-all ${
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {[0, 1, 2, 3, 4, 5].map((num) => (
              <button
                ...
                className={`h-10 rounded-lg font-manrope font-bold text-sm border transition-all ${
                  bedrooms === num
        ...
        <button
          onClick={handleReset}
          className="w-full bg-white border border-[#D4755B] text-[#D4755B] font-manrope font-bold text-base py-4 rounded-xl transition-all hover:bg-[#D4755B] hover:text-white lg:sticky lg:bottom-0"
        >
          Reset Filters
        </button>
      </div>
