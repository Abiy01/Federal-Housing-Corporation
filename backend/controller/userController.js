...
const createtoken = (id, rememberMe = false, role = "user") => {
  return jwt.sign(
    { id: id.toString(), role: role || "user" },
...
const login = async (req, res) => {
  try {
    ...
    if (isMatch) {
      const role = Registeruser.role || "user";
      const token = createtoken(Registeruser._id, rememberMe, role);
      return res.json({
        token,
        user: {
...
const verifyEmail = async (req, res) => {
  try {
    ...
    const authToken = createtoken(user._id, true, role);
