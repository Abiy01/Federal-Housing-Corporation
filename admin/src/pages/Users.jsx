...
import {
  ...
  AlertCircle, RefreshCw, UserCheck, Ban, Wrench, DollarSign
} from "lucide-react";
...
const UsersManagement = () => {
  ...
  const handleRoleUpdate = async (user, role) => {
    try {
      setActionLoading(true);
      const response = await apiClient.put(`/api/admin/users/${user._id}/role`, { role });
            ...
            <table className="w-full">
              <thead className="bg-[#F5F1E8] border-b border-[#E6D5C3]">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-[#1C1B1A]">Role</th>
                ...
                {users.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {user.role === 'maintenance' ? 'Maintenance' : user.role === 'finance' ? 'Finance' : 'User'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {user.status === 'active' && user.role === 'user' && (
                          <button
                            onClick={() => handleRoleUpdate(user, 'maintenance')}
                            disabled={actionLoading}
                            className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors disabled:opacity-50"
                            title="Promote to Maintenance Staff"
                          >
                            <Wrench className="w-4 h-4" />
                          </button>
                        )}
                        {user.status === 'active' && user.role === 'maintenance' && (
                          <button
                            onClick={() => handleRoleUpdate(user, 'user')}
                            disabled={actionLoading}
                            className="px-2.5 py-1.5 text-xs font-medium text-[#5A5856] hover:bg-[#F5F1E8] rounded-lg transition-colors disabled:opacity-50"
                            title="Revoke Maintenance Role"
                          >
                            Revoke
                          </button>
                        )}
                        {user.status === 'active' && user.role === 'user' && (
                          <button
                            onClick={() => handleRoleUpdate(user, 'finance')}
                            disabled={actionLoading}
                            className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors disabled:opacity-50"
                            title="Promote to Finance"
                          >
                            <DollarSign className="w-4 h-4" />
                          </button>
                        )}
                        {user.status === 'active' && user.role === 'finance' && (
                          <button
                            onClick={() => handleRoleUpdate(user, 'user')}
                            disabled={actionLoading}
                            className="px-2.5 py-1.5 text-xs font-medium text-[#5A5856] hover:bg-[#F5F1E8] rounded-lg transition-colors disabled:opacity-50"
                            title="Revoke Finance Role"
                          >
                            Revoke
                          </button>
                        )}
